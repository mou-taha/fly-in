...
class Path:
    def __init__(self, zones: List[Zone]) -> None:
        ...
        self.zones = zones
        self.map: Map = None
        # per-path drone storage: map each Zone in this path to its own list of drones
        self.zone_drones: dict[Zone, List[Drone]] = {zone: [] for zone in zones}
    ...
    def turn(self) -> str:
        ...
        for index, target_zone in reversed(list(enumerate(self.zones))):
            ...
            if target_zone.category != ZoneCategory.START_HUB:
                ...
                # if there is a connection between previous_zone and target_zone,
                # first try to move any drones waiting on that connection into the target zone
                traversal_conn = None
                if isinstance(previous_zone, Zone):
                    traversal_conn = next(
                        (cn for cn in previous_zone.connections if cn.zone.name == target_zone.name),
                        None,
                    )
                if traversal_conn is not None:
                            turns += self._flush_connection_to_zone(traversal_conn, target_zone)
    ...
    def _flush_connection_to_zone(self, connection: Connection, target_zone: Zone) -> str:
        """Try to move drones waiting on a connection into the target zone when capacity allows.

        Drones waiting on restricted zones are stored in `Connection.drones`. This method
        moves as many as allowed by the target zone's available capacity (taking into account
        the map's canonical occupancy) into the path's per-zone list and updates the map.
        """
        ...
        if self.map is not None:
            map_zone = next((z for z in self.map.zones if z.name == target_zone.name), None)
            target_capacity = map_zone.available_capacity() if map_zone is not None else target_zone.available_capacity()
        else:
        ...
        # only move drones that were NOT added to this connection in the current turn
        if self.map is not None:
            added_turns = self.map._connection_added_turn.get(connection, {})
            eligible = [d for d in connection.drones if added_turns.get(d, -1) < self.map.current_turn]
        else:
            eligible = list(connection.drones)
    ...
    def __move_drone(
        ...
        prev_zone = previous.zone if isinstance(previous, Connection) else previous
        ...
        # find the canonical zone object in the map and use its available_capacity()
        map_zone = next((z for z in self.map.zones if z.name == target_zone.name), None)
        if map_zone is None:
        ...
        if target_capacity > 0 and len(prev_list) > 0:
            nb_drones_to_move: int = len(prev_list) if len(prev_list) <= target_capacity else target_capacity

            # check if connection supports the number of drones
            # that will traverse to target zone
            if isinstance(prev_zone, Zone):
                traversal: Connection = next(
                    cn
                    for cn in prev_zone.connections
                    if cn.zone.name == target_zone.name
                )
                # enforce shared connection capacity across paths using map connection usage
                if self.map is not None:
            ...
            # reserve connection capacity immediately so subsequent paths in this turn
            # see the reduced available capacity
            ...
            # If the target is a restricted zone, drones must wait on the connection
            # until the zone has capacity; place them on the connection instead of
            # directly into the zone.
            if target_zone.type == ZoneType.RESTRICTED and traversal_conn is not None:
                # place drones on the connection (they are waiting to enter)
                traversal_conn.drones.extend(drone_to_move)
                if self.map is not None:
                    # mark these drones as added on the current turn so they won't be flushed immediately
                    self.map.mark_connection_newly_added(traversal_conn, drone_to_move)
                ...
                # add to target per-path list and remove from previous per-path list
                self.zone_drones.setdefault(target_zone, []).extend(drone_to_move)
                for drone in drone_to_move:
                    drone.current_place = target_zone
